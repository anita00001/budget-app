...
RSpec.describe CashFlowsController, type: :controller do
  ...
  describe 'POST #create' do
    it 'creates a new cash_flow and redirects to category show page' do
      expect do
        post :create,
             params: { cash_flow: { name: 'Clothing', amount: 30, author_id: @user1.id, categories: [@category] },
    ...
    it 'does not create a new cash_flow and renders new template' do
      expect do
        post :create,
             params: { cash_flow: { name: '', amount: 30, author_id: @user1.id, categories: [@category] },
                       category_id: @category.id }
      end.to change(CashFlow, :count).by(0)
      expect(response).to render_template('new')
    end
  end
end
